...
results_dir = os.path.join('results', 'knn_outlier_detection')
...
k_values = [5, 10, 15]  # Adjust k to be larger for more robust detection
alpha_values = [0.01, 0.05]  # Use smaller alpha to make outlier detection stricter
    ...
    for alpha in alpha_values:
        for h in h_values:
            ...
            for bus in bus_numbers:
                ...
                anomaly_detected_bus = outliers[bus]
                if anomaly_detected_bus.any():
                    pred_labels[np.argmax(anomaly_detected_bus):] = 1
...
results_df.to_csv(os.path.join(results_dir, 'knn_outlier_detection_results.csv'), index=False)
print(results_df)
